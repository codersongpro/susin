...
# 명단 표에 흔히 끼는 말들. 사람 이름으로 보면 안 된다.
# '학성초등학교 / 교사 / 송동석' 같은 세 칸짜리 표에서 '교사' 가 이름 자리를 차지하고
# 정작 '송동석' 이 버려지는 일이 있었다.
TITLE_WORDS = frozenset({
    # 학교
    '교장', '교감', '교사', '수석교사', '담임', '부담임', '원장', '원감',
    '보건교사', '영양교사', '사서교사', '전문상담교사', '특수교사', '기간제교사',
    '영양사', '사서', '상담사', '전문상담사', '실무사', '교육실무사', '조리사',
    '조리원', '행정실장', '행정직원', '공무직', '강사', '방과후강사', '돌봄전담사',
    # 교육청·기관
    '교육감', '부교육감', '교육장', '국장', '과장', '관장', '소장', '단장', '부장',
...
def is_person_name(s: str) -> bool:
    """2~4자 한글, 학교명도 직위도 머리글도 아닌 것"""
    s = s.strip()
    if s in NON_NAME_WORDS:
        return False
...
def _org_from_line(line: str):
    ...
    # 직위·표 머리글은 기관일 수 없다. 먼저 걷어낸다.
    # 그러지 않으면 '소속 / 직위 / 성명' 머리글 줄에서 '성명' 이 기관 후보로 남는다.
    tokens = [t for t in _tokenize(line) if t and t not in NON_NAME_WORDS]
    if not tokens:
        return None
